...
```bash
head -1 /root/projects/superpowers/DaysActivity.md 2>/dev/null
```
...
```bash
head -80 /root/projects/superpowers/DaysActivity.md
```
...
```bash
cat /root/projects/superpowers/CurrentStatus.md
```
...
```bash
tail -30 /root/projects/superpowers/.beads/issues.jsonl | jq -r 'select(.status == "open") | [.id, .type, .title] | @tsv' | column -t
```
...
CM_PORT=$(grep -E '^CM_PORT=' /root/projects/superpowers/factory/factory.env 2>/dev/null | cut -d'"' -f2)
...
jq -r 'select(.status == "open") | [.id, .title, .metadata.team // "none", .metadata.priority // "none"] | @tsv' \
  /root/projects/superpowers/.beads/issues.jsonl | column -t
```
...
jq -r 'select(.status == "open" and .metadata.team) | .metadata.team' \
  /root/projects/superpowers/.beads/issues.jsonl | sort | uniq -c
```
...
**Write to**: `/root/projects/superpowers/session-briefing.md`
